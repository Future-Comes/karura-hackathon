...
export async function addPoolVolume(
    ...
    const volumeDayUSD = await getPriceUSD(store, currencyZero, volumeDay);
...
export async function addPoolLiquidity(
    ...
    const id = pool.id + '-' + timestamp;
    const usdPriceZero = await getPriceUSD(store, pool.currencyZero, balanceZero);
    const usdPriceOne = await getPriceUSD(store, pool.currencyOne, balanceOne);
